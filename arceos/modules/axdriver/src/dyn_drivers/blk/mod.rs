...
impl BlockDriverOps for Block {
    ...
    if buf.len() % block_size != 0 {
        return Err(DevError::InvalidParam); // 或 OperationNotSupported
    }
    
    let blk_count = buf.len() / block_size;
    if blk_count == 0 {
    ...
    for (block, chunk) in blocks.into_iter().zip(buf.chunks_mut(block_size)) {
        let block = block.map_err(maping_blk_err_to_dev_err)?;
        if block.len() != chunk.len() {
            return Err(DevError::Io);
        }
